...
public:
  ...
  Feature(int i) { this->id = i; }
  bool operator<(const Feature &rhs) const { return this->id < rhs.id; }
};
...
public:
   ...
   */
  void addFeature(Feature f) { features.insert(f); }

  bool operator<(const Room &rhs) const { return this->id < rhs.id; }
};
...
public:
   ...
   * @return void
   */
  void addAtendee(Student s) { atendees.insert(s); }
   ...
   */
  int getNumberOfAtendees() { return atendees.size(); }
   ...
   * @param f The feature to add.
   */
  void addFeature(Feature f) { this->features.insert(f); }
